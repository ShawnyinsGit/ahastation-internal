...
test('RK3588 release script keeps build, verification and manifest gates in order', async () => {
  ...
  assert.match(command, /prebuild:whisper:linux-arm64/);
  assert.match(command, /assert-whisper-assets/);
...
test('arm64 CI is built inside Debian 11 and signed releases depend on it', async () => {
  ...
  assert.match(workflow, /image:\s*debian:11/);
...
test('board gate accepts packaged runtimes but still requires external Kimi', async () => {
  ...
  assert.match(gate, /check_version kimi 0\.24\.1/);
});
